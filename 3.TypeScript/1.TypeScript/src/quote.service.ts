import { IQuote} from './quote.model'; //importing quote interface

export class QuoteService {
  //TypeScript:
 private quotes = sampleQuotes; //private field
  //instead of:
  // constructor() {
  //   this.quotes = sampleQuotes; //public field
  // }
  
  //private method:
  private getRandomQuote() : IQuote { //return an IQuote model
    const randomIndex = Math.floor(Math.random() * this.quotes.length);
    return this.quotes[randomIndex];
  }
  //public method by default:
  generateRandomQuotes(delay:number, callback:(quote : IQuote)=>void) { //declaring that delay of type number,callback function that get argument and not return value 
              //instead of: (delay,callback)
...
const sampleQuotes : IQuote[] = [ //array from type IQuote
  {
